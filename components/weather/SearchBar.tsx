  ...
  return (
    <div className="space-y-2" ref={rootRef}>
      <form onSubmit={event => { event.preventDefault(); if (results[0]) select(results[0]); }} className="relative">
        <SearchIcon className="pointer-events-none absolute left-3.5 top-1/2 h-[18px] w-[18px] -translate-y-1/2 text-slate-500" />
        ...
        <input
          ...
          autoComplete="off"
          className={`h-11 w-full rounded-xl border border-white/[0.09] py-2.5 pl-10 pr-12 text-sm text-white outline-none placeholder:text-slate-500 focus:ring-1 ${classes.ring} ${isAmoled ? 'bg-black' : 'bg-[#111419]'}`}
        />
        <button type="submit" aria-label="Pesquisar" className={`absolute right-1 top-1 flex h-9 w-9 items-center justify-center rounded-lg ${classes.bg} text-white transition-opacity hover:opacity-90`}>
          <SearchIcon className="h-4 w-4" />
        </button>

        {open && (
          <div id="city-search-results" className={`absolute left-0 right-0 top-[calc(100%+0.4rem)] z-50 max-h-72 overflow-y-auto rounded-xl border border-white/10 p-1.5 shadow-xl ${isAmoled ? 'bg-black' : 'bg-[#15191f]'}`} role="listbox">
            {loading ? (
              ...
              <button key={`${city.lat}-${city.lon}-${index}`} type="button" role="option" aria-selected="false" onClick={() => select(city)} className="flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-left transition-colors hover:bg-white/[0.05]">
                <MapPinIcon className="h-4 w-4 flex-none text-slate-500" />
      ...
      <div className="scroll-fade flex gap-2 overflow-x-auto pb-1" aria-label="Atalhos de localização">
        {showGeolocate && (
          <button type="button" onClick={onGeolocate} className="flex flex-none items-center gap-1.5 rounded-lg px-2 py-1 text-xs text-slate-400 hover:bg-white/[0.05] hover:text-white">
            <MapPinIcon className={`h-3.5 w-3.5 ${classes.text}`} /> Minha localização
          </button>
        )}
        {quickCities.map(city => <button key={city.name} type="button" onClick={() => select(city)} className="flex-none rounded-lg px-2 py-1 text-xs text-slate-500 hover:bg-white/[0.05] hover:text-white">{city.name}</button>)}
      </div>
    </div>
  );
};
