...
const MapView: React.FC<MapViewProps> = ({ lat, lon }) => {
    ...
    useEffect(() => {
        ...
        const reliefLayer = L.tileLayer('/.netlify/functions/weather?endpoint=relief&z={z}&x={x}&y={y}', {
            ...
            <div className="absolute top-20 right-4 z-[401]">
                {isLayerMenuOpen && (
                    <div className="bg-white/80 backdrop-blur-md border border-gray-300/50 rounded-xl shadow-lg p-2 flex flex-col gap-1 w-48">
                        ...
                        <LayerButton icon={<DropletsIcon className="w-5 h-5 text-blue-500"/>} label="Precipitação" onClick={() => toggleWeatherLayer('PR0')} />
                        <LayerButton icon={<GaugeIcon className="w-5 h-5 text-indigo-500"/>} label="Pressão" onClick={() => toggleWeatherLayer('APM')} />
                ...
                <button 
                    ...
                    aria-label="Controle de sobreposições do mapa"
                >
